...
import { ExclamationTriangleIcon, EnvelopeIcon, XMarkIcon } from '@heroicons/react/24/solid'
...
const ALL_MESSAGES: Message[] = [
  { sender: 'Farhan', sentDate: new Date(), delivered: new Date(), message: 'Hello, Please help me decide a suitable crop?', seen: new Date() },
  { sender: 'Waheed', sentDate: new Date(), delivered: new Date(), message: 'Can you tell me how to set up a new Crop?' },
  { sender: 'Nmberdar', sentDate: new Date(), delivered: new Date(), message: 'Can you tell me how to set up a new Crop?' },
]
      ...
      <VCard className="cropCard">
        ...
        <Formik
          ...
          onSubmit={(values, action) => {
            setCropsList([...cropList, { id: (Math.random() * 10).toString(), ...values }])
        ...
        <Line />
        <ViewList onClick={() => setShowAllCrops(!showAllCrops)}>All Crops</ViewList>
        <div>
          {showAllCrops &&
            cropList.map((c) => (
              <div>
      ...
      <VCard>
        <HCard onClick={() => setShowChatBox(!showChatBox)}>
          ...
          <div className="MessageEnvelope">
            ...
            />
            <span className="MessageCount">{ALL_MESSAGES.filter((m) => m.seen === undefined).length}</span>
          </div>
        </HCard>
        {showChatBox && (

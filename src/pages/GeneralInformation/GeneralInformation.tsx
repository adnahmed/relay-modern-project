...
const GeneralInformation: FC<GeneralInformationProps> = () => {
  ...
  const changeIrrigationSources = useUserStore(state => state.changeIrrigationSource)
  ...
  useEffect(() => {
    setSoilType({ target: { value: SoilType[0].label } })
  })
  return (
    <div className="GeneralInformation">
      <div id="menu-buttons">
        <div className="next button">
          <Link to="dashboard"><GlowyButton>Next Page</GlowyButton></Link>
        </div>
        {!editing ? (
      ...
      <div id="owner">
        Farm Owner Name:
        {editing ? <input value={farmOwner} onChange={setFarmOwner}></input> : <div className="information"> {farmOwner} </div>}
      </div>
      <div id="total-land">
        Total Land:
        {editing ? <input value={totalLand} onChange={setTotalLand}></input> : <div className="information">{totalLand}</div>}
        {' Acres'}
      </div>
      <div id="own-land">
        Own Land:
        {editing ? <input value={ownLand} onChange={setOwnLand}></input> : <div className="information">{ownLand}</div>}
        {' Acres'}
      </div>
      <div id="own-land">
        Leased Land:
        {editing ? <input value={leasedLand} onChange={setLeasedLand}></input> : <div className="information">{leasedLand}</div>}
        {' Acres'}
      </div>
      <div id="own-land">
        <>
      ...
      <div>
        Irrigation Source
          {editing ? 
        <>
        <input className="information"  value={irrigationSourceOne.one} onChange={(event) => {changeIrrigationSources('one', event.target.value)}} />
        <input className="information" value={irrigationSourceOne.two} onChange={(event) => {changeIrrigationSources('two', event.target.value) }} />
</>
          : <>
      ...
      <div>
        Machinery Available
        <input className="information"  value={machinery[0]} onChange={(event) => {setMachinery(0, event.target.value)}} />
        <input className="information" value={machinery[1]} onChange={(event) => { setMachinery(1, event.target.value) }} />
        <input className="information"  value={machinery[2]} onChange={(event) => {setMachinery(2, event.target.value)}} />
        <input className="information" value={machinery[3]} onChange={(event) => { setMachinery(3, event.target.value) }} />
      </div>
      <div>
